<%@ include file="session.jsp"%>

<!--
Author: W3layouts
Author URL: http://w3layouts.com
License: Creative Commons Attribution 3.0 Unported
License URL: http://creativecommons.org/licenses/by/3.0/
-->
<!DOCTYPE html>
<html>
<head>
<title>IIDPS: User</title>
<link href="css/bootstrap.css" rel="stylesheet">
<link href="css/style.css" rel="stylesheet" type="text/css" media="all" />
<!-- for-mobile-apps -->
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="keywords" content="Spring Spa Responsive web template, Bootstrap Web Templates, Flat Web Templates, Andriod Compatible web template,Smartphone Compatible web template, free webdesigns for Nokia, Samsung, LG, SonyErricsson, Motorola web design" />
<script type="application/x-javascript"> addEventListener("load", function() { setTimeout(hideURLbar, 0); }, false); function hideURLbar(){ window.scrollTo(0,1); } </script>
<!-- //for-mobile-apps -->	
<script type="text/javascript" src="js/modernizr.custom.21750.js"></script>
<script src="js/modernizr.custom.js"></script>
<script src="js/jquery.min.js"> </script>
</head>
    <body>
<div class="header">
        <div class="container">
            <div id="bx-wrapper" class="bx-wrapper">
				<!--div class="bx-thumbs bx-thumbs1">
                    <a href="#" class="bx-thumbs-current" style="background-image:url(images/6.jpg)"></a>
                    <a href="#" style="background-image:url(images/7.jpg)"></a>
                    <a href="#" style="background-image:url(images/4.jpg)"></a>
                    <a href="#" style="background-image:url(images/3.jpg)"></a>
                    <a href="#" style="background-image:url(images/2.jpg)"></a>
                </div-->
                <div class="bx-container">
                    <img src="images/bg.jpg" alt="bg" title="Security"/>
                </div>
                <div class="bx-overlay"></div>
                      <div class="side-viwer slider-viwer1">
			                    	<h1><a href="#">IIDPS</a></h1>
			                    	
			                    
			                    <div class="top-menu">
							 <span class="menu"> </span> 
							 <ul>
							 	<nav class="cl-effect-1" id="cl-effect-1">
							 <li><a href="u_home.jsp">HOME</a>

							 </li>
				   		  	 
							 
							 <li><a  href="upload.jsp">DATA UPLOAD</a></li>
							 <li><a  href="viewownfiles.jsp">VIEW FILES</a></li>
							 <li><a  href="search.jsp">SEARCH</a></li>
							 <li><a  href="inbox.jsp">INBOX</a></li>
							 <li><a  href="viewprofile.jsp">VIEW PROFILE</a></li>
							<li>
														 <a href="#" onclick="window.open('sc.jsp', 'newwindow', 'width=450, height=500'); return false;">
                                                             <!--img src="images/window-system-icon.png" width="64" height="54" border="0" alt=""-->
                                                             SYSTEM CALLS
                                                         </a> 

							</li>
							 <li><a href="logout.jsp">LOGOUT</a></li>
								 </nav>
							 </ul>
						 </div>
						 	 <!-- script-for-menu -->
		 <script>
				$("span.menu").click(function(){
					$(".top-menu ul").slideToggle("slow" , function(){
					});
				});
		 </script>
		 <!-- script-for-menu --> 
						 		 <div class="social-icons">
									
												 
								 </div>
					</div>
					<div class="about">
						<div class="about-top">
							<!-- <h2><font size="" color="#048ee1"><strong>An Internal Intrusion Detection and Protection System by Using Data Mining and Forensic Techniques</strong></font></h3>
 -->
 							<!--img src="images/logo.png"  border="0" alt="">

 <hr-->


